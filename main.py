...
product_name = "Bedsure Cat Beds for Indoor Cats"
description = f"""
                ...
                have a distinctly sci-fi feel, with a focus on action and adventure. The brief should comprehensively 
                outline our objectives, the target audience, the tone, and the key messages. Work with the researcher to gather any 
                necessary insights.
                """
create_creative_brief = tasks.create_creative_brief(creative_director, description)
create_seo_brief = tasks.create_seo_brief(seo_specialist, [create_creative_brief])
create_social_media_plan = tasks.create_social_media_plan(
    social_media_manager, [create_seo_brief]
)
write_ad_copy = tasks.write_ad_copy(
    copywriter, [create_creative_brief, create_seo_brief]
)
write_social_media_posts = tasks.write_social_media_posts(
    copywriter, [create_social_media_plan, create_seo_brief]
)
write_story = tasks.write_story(
    writer, [create_creative_brief, create_seo_brief, write_ad_copy]
)
